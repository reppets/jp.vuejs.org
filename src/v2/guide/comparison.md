...
React では、すべてのものが JavaScript です。JSX で表現される HTML 構造だけでなく、CSS の管理も JavaScript の中に表されるのが最近の傾向です。このアプローチには利点もありますが、さまざまなトレードオフがあり、必ずしもすべての開発者にとって有利になるわけではないように思われます。

Vue は古典的な Web 技術を活用し、それに基づいて構築されています。その意味を示すために、いくつかの例を取り上げます。

#### JSX vs Templates

React では、すべてのコンポーネントの UI は JSX を使った描画 (render) 関数の中で表現されます。JSX とは JavaScript の中で用いることのできる、宣言的で XML のような構文です。

JSX を使った描画関数にはいくつかの優位な点があります：

- 完全なプログラミング言語 (JavaScript) の力を活用してビューを構築することができます。これには、一時変数、制御構造、スコープ内の JavaScript 値の直接参照が含まれます。
- JSX のためのツールのサポート（例えば、Lint、型チェック、エディタのオートコンプリート）は現在 Vue のテンプレートで利用可能なものよりもいくつかの点で優れています。

Vue でも、[描画関数](render-function.html)をそなえ、さらには [JSX をサポート](render-function.html#JSX)しています。なぜなら、時折その力が本当に必要になることがあるためです。しかし、よりわかりやすい代替手段として、テンプレートをデフォルトの"方式"として提案します。Valid な HTML であればどのようなものも valid な Vue テンプレートとなり、そのことによるいくつかの利点があります:

- HTML を扱っている多くの開発者にとって、テンプレートの読み書きは、より自然に感じられるでしょう。この点の好みは多少主観的な物ですが、それによって開発者の生産性が上がるのであれば、生産性向上という利益は客観的な物です。
- HTML ベースのテンプレートは、既存のアプリケーションから、段階的に Vue のリアクティブ機能を利用するよう移行することを、ずっと簡単にします。
